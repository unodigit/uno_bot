#!/usr/bin/env python3
"""Test script to verify TodoListMiddleware functionality using default model."""
...
async def test_todolist_middleware_with_default_model():
    """Test if TodoListMiddleware functions are available with default model."""
    try:
        from deepagents import create_deep_agent
        from deepagents.backends import CompositeBackend, FilesystemBackend

        print("=== Testing TodoListMiddleware with Default Model ===")
        ...
        # Create agent with default model (no model parameter = use default)
        ...
        print("✓ Agent created successfully with default model")
        ...
        print("\n=== TodoListMiddleware Verification ===")
        if hasattr(agent, 'write_todos') and hasattr(agent, 'read_todos'):
            print("✅ TodoListMiddleware functionality is WORKING")
            print("   - write_todos tool available for task planning")
            print("   - read_todos tool available for progress tracking")
            return True
        else:
...
if __name__ == "__main__":
    import asyncio
    success = asyncio.run(test_todolist_middleware_with_default_model())
